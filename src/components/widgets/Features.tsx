    ...
    [
      {
        ...
        description:
          "Als Spezialist auf dem Gebiet der Bestattungsvorsorge ist der WIENER VEREIN Ihr kompetenter Ansprechpartner. Wir konzentrieren uns ausschließlich auf die umfassende Vorsorge für den Todesfall.",
      ...
      <div class="px-4 py-16 mx-auto max-w-6xl lg:px-8 lg:py-20">
        ...
        <div class="grid mx-auto space-y-6 md:grid-cols-2 md:space-y-0 justify-center md:justify-between">
          {items.map((subitems, index) => (
            <div
              key={`item-${index}`}
              class="space-y-8 sm:px-8 flex flex-col justify-between"
            >
              {subitems.map(({ title, description }) => (
                <div
                  key={title}
                  class="flex flex-col max-w-md items-center p-6 bg-white rounded-xl text-center h-full"
                >
                  <div class="mb-4 mr-4">
