  ...
  const items = [
    [
      ...
      },
      {
        ...
        list: ["Auch wenn keine Prämien mehr bezahlt werden"],
        icon: IconPuzzle,
      },
      {
    ...
    <section class="bg-white">
      <div class="container px-6 py-10 mx-auto max-w-screen-xl">
        <h2 class="text-2xl font-semibold text-gray-800 lg:text-3xl">
          Unsere{" "}
          <span class="underline decoration-third">Alleinstellungs-</span>
          <br />
          <span class="underline decoration-third">merkmale</span>
        </h2>

        <p class="mt-4 text-gray-500 xl:mt-6 ">
          Einzigartige Vorteile: Sofortschutz, lebenslanger Schutz und
          Ausrichtung nach persönlichen Wünschen.
        </p>
        <div class="grid grid-cols-1 gap-8 mt-8 xl:mt-12 xl:gap-12 md:grid-cols-2 xl:grid-cols-3">
          {items.map((subItem) =>
            subItem.map(({ title, list, icon: Icon }) => (
              <div
                key={`index-${title}`}
                class="p-8 space-y-3 border-2 border-third rounded-xl"
              >
                {Icon && <Icon />}

                <h3 class="text-xl font-semibold text-gray-700 capitalize ">
                  {title}
                </h3>
                <ol class="list-disc">
                  {list.map((listitem) => (
                    <li key={`index-${listitem}`}>
                      <p class="text-gray-500">{listitem}</p>
                    </li>
                  ))}
                </ol>
              </div>
